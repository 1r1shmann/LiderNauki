<ul class="navbar-nav mr-auto ">
    <?php
        ...
        if (isset($_SESSION['auth'])) {
            ...
            foreach (app\core\Config::MENU_AUTH as $key => $item) {
                ...
                if (empty($item['visibility']) && empty($item['banned'])){
                    $visibility_flag = true;
                } else {
                    foreach ($user->userrules as $rule){
                        if (in_array($rule->rule_name, $item['banned'])){
                            $visibility_flag = false;
                            break;
                        }
                        else if(in_array($rule->rule_name, $item['visibility'])){
                            $visibility_flag = true;
                            break;
                        }  
                    }
                ...
                echo '<li class="nav-item ', $subClass, $active,'">';
                    if(!$hasSub){
                        ...
                        foreach ($item['sub'] as $subkey => $subitem){
                            ...
                            if (empty($subitem['visibility']) && empty($subitem['banned'])){
                                $visibility_flag = true;
                            } else {
                                $user = \app\models\User::find('first', [
                                    'conditions' => ['id=?', $_SESSION['user']['id']]
                                ...
                                foreach ($user->userrules as $rule){
                                    if (in_array($rule->rule_name, $subitem['banned'])){
                                        $visibility_flag = false;
                                        break;
                                    }
                                    else if(in_array($rule->rule_name, $subitem['visibility'])){
                                        $visibility_flag = true;
                                        break;
                                    }  
                                }
            ...
            foreach (app\core\Config::MENU_NON_AUTH as $key => $item) {
                $active = ($url === $item['url']) ? ' active' : '';
                $hasSub = isset($item['sub']) && is_array($item['sub']);
                $subClass = $hasSub ? 'dropdown' : '';
